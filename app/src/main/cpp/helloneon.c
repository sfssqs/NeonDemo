...
jstring
Java_com_example_helloneon_HelloNeon_compareNeon(JNIEnv *env, jobject thiz,
                                                 jbyteArray buffer,
                                                 int width,
                                                 int height) {
    int pixels = width * height;

    unsigned char *argb = (unsigned char *) malloc(pixels * 4);
    unsigned char *yuv = (unsigned char *) malloc(pixels * 3);

    (*env)->GetByteArrayRegion(env, buffer, 0, pixels, argb);


    char *str;
    char b[512] = "hello neon test\n";


    double t2 = now_ms();
    for (int i = 0; i < 100; i++) {
#ifdef HAVE_NEON
        reference_convert(yuv, argb, width * height);
#endif
    }
    double t3 = now_ms();
    double time_neon = t3 - t2;
    asprintf(&str, "reference_convert, use time : %gms\n", time_neon);
    strlcat(b, str, sizeof b);
    free(str);

    double t6 = now_ms();
    for (int i = 0; i < 100; i++) {
#ifdef HAVE_NEON
        neon_convert(yuv, argb, width * height);
#endif
    }
    double t7 = now_ms();
    double time_float = t7 - t6;
    asprintf(&str, "neon_convert, use time : %gms\n", time_float);
    strlcat(b, str, sizeof b);
    free(str);
    ...
    for (int i = 0; i < 100; i++) {
#ifdef HAVE_NEON
        neon_asm_convert(yuv, argb, width * height);
#endif
    }
    double t1 = now_ms();
    double time_c = t1 - t0;
    asprintf(&str, "neon_asm_convert, use time : %gms\n", time_c);
    strlcat(b, str, sizeof b);
    free(str);
 ...
 */
jstring
Java_com_example_helloneon_HelloNeon_stringFromJNI(JNIEnv *env, jobject thiz) {
    char *str;
